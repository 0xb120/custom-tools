...
- The container imports **nothing** from the host's `~/.claude` / `~/.codex` beyond those two credential files, so it starts with no plugins and no marketplaces. Plugins, marketplaces and MCP servers are declared per engagement: `.mcp.json` for Claude (project scope, auto-approved via `enableAllProjectMcpServers`), `codex mcp add` in `postCreateCommand` for Codex (it ignores project-scoped `mcp_servers`), and plugins installed from inside the container with `claude plugin install <plugin>@<marketplace> --scope project`, which records the choice in the engagement's own `.claude/settings.json`. Don't reintroduce a host-config seed: an enabled host plugin can inject a `SessionStart` PATH shim that breaks the `python3 db/ptctl.py` hooks (`modern-python` does exactly that whenever `uv` is present, and `uv` ships in the `base` group) and can drag in MCP servers pointing at host-only binaries. Plugin enablement is driven solely by `enabledPlugins` — a plugin with no entry is inactive; precedence is user < project < local < flag < policy.
