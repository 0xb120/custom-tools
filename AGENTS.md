...
- `newPT.sh` scaffolds agent configs for **both** Codex (`.Codex/`) and Codex (`.codex/`). The two payload-compatible hooks (`log-command.sh`, `render-after-db.sh`) live once in `org/templates/hooks/` and are copied into both agents' `hooks/` dirs; `check-report-format.sh` is Codex-only (`org/templates/Codex/hooks/`) since Codex edits go through `apply_patch` (no `file_path`). Codex config is `.codex/config.toml` (`approval_policy=never` + `sandbox_mode=danger-full-access`, the bypassPermissions analog) + `.codex/hooks.json`. Container auth is seeded by `org/seed-codex-env.sh` (mirrors `seed-Codex-env.sh`); launch via `./yolo-codex.sh`.
